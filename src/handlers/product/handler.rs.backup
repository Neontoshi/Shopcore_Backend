use axum::{
    extract::{State, Path, Query, Extension},
    Json,
};
use uuid::Uuid;
use crate::app::state::AppState;
use crate::dtos::{CreateProductRequest, UpdateProductRequest, ProductFilter, ProductResponse};
use crate::services::ProductService;
use crate::errors::AppError;
use crate::middleware::auth::AuthUser;
use validator::Validate;

pub async fn create_product(
    State(state): State<AppState>,
    Extension(auth_user): Extension<AuthUser>,
    Json(req): Json<CreateProductRequest>,
) -> Result<Json<ProductResponse>, AppError> {
    if auth_user.role != "admin" {
        return Err(AppError::forbidden("Admin access required"));
    }
    if let Err(errors) = req.validate() {
        return Err(AppError::validation(errors.to_string()));
    }
    let product = ProductService::create_product(state.get_db_pool(), req).await?;
    Ok(Json(product))
}

pub async fn get_product(
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<ProductResponse>, AppError> {
    let product = ProductService::get_product(state.get_db_pool(), &id).await?;
    Ok(Json(product))
}

pub async fn update_product(
    State(state): State<AppState>,
    Extension(auth_user): Extension<AuthUser>,
    Path(id): Path<Uuid>,
    Json(req): Json<UpdateProductRequest>,
) -> Result<Json<ProductResponse>, AppError> {
    if auth_user.role != "admin" {
        return Err(AppError::forbidden("Admin access required"));
    }
    let product = ProductService::update_product(state.get_db_pool(), &id, req).await?;
    Ok(Json(product))
}

pub async fn delete_product(
    State(state): State<AppState>,
    Extension(auth_user): Extension<AuthUser>,
    Path(id): Path<Uuid>,
) -> Result<Json<crate::utils::MessageResponse>, AppError> {
    if auth_user.role != "admin" {
        return Err(AppError::forbidden("Admin access required"));
    }
    ProductService::delete_product(state.get_db_pool(), &id).await?;
    Ok(Json(crate::utils::MessageResponse::new("Product deleted successfully")))
}

pub async fn list_products(
    State(state): State<AppState>,
    Query(params): Query<ProductFilter>,
) -> Result<Json<Vec<ProductResponse>>, AppError> {
    let limit = params.page_size.unwrap_or(20) as i64;
    let offset = params.page.unwrap_or(1) as i64 * limit - limit;
    let products = crate::repositories::ProductRepository::search(
        state.get_db_pool(),
        params.query.as_deref(),
        params.category_id,
        params.min_price,
        params.max_price,
        params.is_active,
        limit,
        offset,
    ).await?;
    Ok(Json(products.into_iter().map(|p| p.into()).collect()))
}

pub async fn list_categories(
    State(state): State<AppState>,
) -> Result<Json<Vec<crate::dtos::CategoryResponse>>, AppError> {
    let categories = crate::repositories::CategoryRepository::find_all_active(state.get_db_pool())
        .await
        .map_err(|_| AppError::internal_server_error())?;
    Ok(Json(categories.into_iter().map(|c| c.into()).collect()))
}

pub async fn get_featured_products(
    State(state): State<AppState>,
) -> Result<Json<Vec<ProductResponse>>, AppError> {
    let products = crate::repositories::ProductRepository::search(
        state.get_db_pool(),
        None,
        None,
        None,
        None,
        Some(true),
        8,
        0,
    )
    .await
    .map_err(|_| AppError::internal_server_error())?;
    Ok(Json(products.into_iter().map(|p| p.into()).collect()))
}

pub async fn search_products(
    State(state): State<AppState>,
    Query(params): Query<std::collections::HashMap<String, String>>,
) -> Result<Json<Vec<ProductResponse>>, AppError> {
    let query = params.get("q").map(|s| s.as_str());
    let products = crate::repositories::ProductRepository::search(
        state.get_db_pool(),
        query,
        None,
        None,
        None,
        Some(true),
        20,
        0,
    )
    .await
    .map_err(|_| AppError::internal_server_error())?;
    Ok(Json(products.into_iter().map(|p| p.into()).collect()))
}