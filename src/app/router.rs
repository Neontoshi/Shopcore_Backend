...
// use std::sync::Arc;
// use tower_governor::{governor::GovernorConfigBuilder, GovernorLayer};  // COMMENTED OUT - Rate limiting disabled for development
use crate::handlers::{
    address, admin, auth, cart, health, order, payments, product, review, user, vendor, webhook,
    wishlist,
};
use crate::middleware::auth::auth_middleware;
// use crate::middleware::user_rate_limiter::UserIdKeyExtractor;  // COMMENTED OUT - Rate limiting disabled for development
    ...
    */
    ...
    let public_routes = Router::new()
        ...
        .route(
            ...
            get(product::get_featured_products),
        )
        ...
        .route(
            ...
            get(review::get_product_reviews),
        )
        .route(
            "/api/reviews/{review_id}/replies",
            get(review::get_review_replies),
        )
        .route(
            "/api/settings/platform-fee",
            get(admin::get_platform_settings),
        );
    ...
    // .layer(strict_limiter);  // COMMENTED OUT

    let password_reset_routes = Router::new()
        .route("/api/auth/forgot-password", post(auth::forgot_password))
        .route("/api/auth/reset-password", post(auth::reset_password));
    // .layer(password_limiter);  // COMMENTED OUT
    ...
    let protected_routes = Router::new()
        ...
        .route(
            "/api/reviews/user/{product_id}",
            get(review::check_user_review),
        )
        .route(
            "/api/reviews/{review_id}/helpful",
            post(review::mark_review_helpful),
        )
        .route(
            "/api/reviews/{review_id}/replies",
            post(review::add_reply_to_review),
        )
        .route(
            "/api/reviews/user-votes",
            post(review::get_user_review_votes),
        )
        ...
        .route(
            ...
            get(wishlist::check_in_wishlist),
        )
        .route("/api/wishlist/count", get(wishlist::get_wishlist_count))
        .route(
            "/api/vendor/products",
            get(vendor::get_my_products).post(vendor::create_product),
        )
        .route(
        ...
        .route(
            "/api/vendor/orders/{order_id}/status",
            put(vendor::update_order_status),
        )
        .route("/api/payments/initiate", post(payments::initiate_payment))
        .route(
            "/api/payments/crypto/status/{charge_id}",
            get(payments::get_crypto_status),
        )
        .route(
            "/api/orders/{order_id}/tracking",
            get(order::tracking_handler::get_order_tracking),
        )
    ...
    let admin_routes = Router::new()
        ...
        .route(
            ...
            get(admin::get_shipping_settings).put(admin::update_shipping_settings),
        )
        .route(
            "/api/admin/products/{product_id}/status",
            put(admin::update_product_status),
        )
        .route("/api/admin/inventory", get(admin::get_inventory))
        .route(
            "/api/admin/inventory/adjust",
            post(admin::manual_adjust_stock),
        )
        .route(
            "/api/admin/inventory/low-stock",
            get(admin::get_low_stock_summary),
        )
        .route("/api/admin/stats", get(admin::get_stats))
        .route(
            "/api/admin/vendor/applications",
            get(admin::get_vendor_applications),
        )
        .route(
            "/api/admin/vendor/applications/{application_id}",
            put(admin::review_application),
        )
        ...
        .route(
            "/api/admin/orders/{order_id}/mark-paid",
            put(admin::mark_order_paid),
        )
        .route(
            "/api/admin/orders/{order_id}/tracking",
            post(admin::shipment_handler::add_tracking),
        )
        .route(
            "/api/admin/orders/{order_id}/delivered",
            put(admin::shipment_handler::mark_delivered),
        )
        .route(
            "/api/admin/orders/{order_id}/estimated-delivery",
            put(admin::shipment_tracking_handler::update_estimated_delivery),
        )
        .route(
            "/api/admin/settings/platform-fee",
            put(admin::update_platform_settings),
        )
        .route("/api/admin/vendors", get(admin::get_vendors))
        .route(
            "/api/admin/products/{product_id}/inventory",
            get(admin::get_product_inventory_logs),
        )
        .route(
            "/api/admin/inventory/{product_id}/logs",
            get(admin::get_product_inventory_logs),
        )
    ...
    let checkout_routes = Router::new()
        ...
        // .layer(checkout_limiter)  // COMMENTED OUT
        .layer(middleware::from_fn_with_state(
            state.clone(),
            auth_middleware,
        ));
    ...
    let webhook_routes = Router::new()
        ...
        .route(
            ...
            post(webhook::nowpayments_webhook),
        )
